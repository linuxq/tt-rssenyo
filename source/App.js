...
enyo.kind({
	...
	components:[
		...
		{kind: "Panels", name: "viewPanels", fit: true, classes: "panels-sample-sliding-panels", arrangerKind: "CollapsingArranger", ondragfinish: "verticalswipefinish", ondragstart: "verticalswipestart", wrap: false, components: [
			...
			{name: "left2", kind: "FittableRows", classes: "panels-theme-light", style: "width:260px; background-color:#fff;", components: [
				...
				{name: "left3", kind: "FittableRows", fit: true, classes: "panels-theme-light", components: [
					{kind: "Scroller", name: "scrollermainview", touch:true, fit: true, horizontal:"hidden", classes: "scroller-sample-scroller", components: [
						{kind: "gts.DividerDrawer", name: "categoryHeader", caption: "Categories", open: true, onChange: "resize", components:[
							{kind: "Scroller", name: "scrollercategories", touch:true, fit: false, horizontal:"hidden", classes: "scroller-sample-scroller", components: [
						...
						//{content: "Feeds (Click to add)", name: "feedHeader", ontap: "addFeedClick", style: "font-size: 1.2em; color: #ffffff; background: #000000; font-weight: bold;"},
							{kind: "Scroller", name: "scrollerfeeds", touch:true, fit: false, horizontal:"hidden", classes: "scroller-sample-scroller", components: [
				...
				{kind: "FittableColumns", showing: true, style: "width:100%; height: 60px; background: #252525;", components:[
					...
					{kind: "FittableRows", name: "btnSettings", style: "height: 60px; background: #252525; width: 52px; padding-left: 10px", ontap: "LoginTap", components:[
						{style: "height: 6px"},
						{kind: "onyx.IconButton", src: "assets/bb10settings.png", style: "height: 32px; width: 40px", showing:true},
						{style: "height: 2px"},
						{content: "Settings", style: "height: 15px; color: #ffffff; font-size: 11px; text-align: center"},
			...
			{name: "body", kind: "FittableRows", fit: true, classes: "panels-theme-light", components: [
				...
				{kind: "Scroller", name: "articlePreviewScroller", horizontal:"hidden", fit: true, touch: true, ondragfinish: "titleDragFinish", ondragstart: "titleDragStart", components: [
					{name: "articlePreview", classes: "panels-sample-sliding-content", allowHtml: true, onclick: "catchtaponlink", content: ""}
				]},
				...
				{kind: "FittableColumns", name: "articleviewtoolbar", showing: true, style: "width:100%; height: 60px; background: #252525;", components:[
					...
					{kind: "FittableRows", name: "bb10btnshare", style: "height: 60px; background: #252525; width: 44px; padding-left: 6px", ontap: "shareArticlebb10", components:[
						{style: "height: 6px"},
						{kind: "onyx.IconButton", name: "bb10iconshare", src: "assets/bb10-share32.png", style: "height: 32px; width: 40px", showing:true, ontap: "shareArticlebb10"},
						{style: "height: 2px"},
		...
		{name: "LoginPopup", style: "width:320px; height: auto", classes: "onyx-sample-popup", kind: "onyx.Popup", centered: true, modal: true, floating: true, onShow: "popupShown", onHide: "popupHidden", autoDismiss: false, components: [
			{kind: "Scroller", style: "height: 300px", touch:true, fit:true, horizontal:"hidden", classes: "scroller-sample-scroller", components: [
				...
				{kind: "onyx.Checkbox", name: "useJsonpRequest", showing: false, content: "Use JsonpRequest", style: "width:100%; height:24px; padding:10px 0px 0px 40px;"},
				...
				{kind: "onyx.Groupbox", name: "setupinstapaper", showing: false, style: "width:100%; background-color:#EAEAEA;", components: [
					{kind: "onyx.InputDecorator", components: [
	...
	startapp: function(inSender,inEvent) {
		...
		if (Datum > BetaDate){
			this.debugconsole("BETA abgelaufen");
		...
		*/
		...
		if (gblBB10) {
			...
			this.debugconsole("PLATFORM: Desktop");
			this.setViewDesktop();					
		} else 
		{
			this.setViewDesktop();
	...
	setViewBB10: function(){
			...
			//this.$.bb10TopMenu.setShowing(true);
			this.$.btnSettings.setShowing(false);
	...
	LoginTap: function(inSender, inEvent) {
		...
		if (this.ttrssURL) {
			...
			this.$.autoLockPanels.setValue(true);
		};
		...
		console.log(this.AutoLockPanels);
	...
	getCategories: function (inSender){
		...
		//this.debugconsole(this.ttrss_SID);
		this.setLoadbar(true);
		var getUnreadOnly = this.$.toggleUnread.getValue();
	...
	processGetCategoriesSuccess: function(inEvent) {
		...
		for (var i=0; i<inEvent.length; i++) {
			//this.debugconsole(inEvent[i].title + " - " + inEvent[i].unread);
	...
	getFeeds: function(inSender, inEvent){
		//this.debugconsole(this.$.catID.getValue());
		this.setLoadbar(true);
		var getUnreadOnly = this.$.toggleUnread.getValue();
	...
	processGetHeadlinesSuccess: function(inEvent){
		//this.debugconsole(inEvent.length);
		...
		if (inEvent.length == 0) {
			this.debugconsole("GetHeadlines: NO HEADLINES");
			this.setLoadbar(false);
			this.$.articleRepeater.setCount(0);
			this.$.articleScroller.setScrollTop(0);
			this.$.viewPanels.setIndex(1);
			this.debugconsole("SETPANEL 2");
			this.clickRefresh();
			return;
		} else {
			for (var i=0; i<inEvent.length; i++) {
	...
	processGetArticleSuccess: function(inEvent){
		//this.debugconsole(inEvent[0].content);
		var TextHelp = "";
		...
		this.debugconsole(inEvent[0]);
		this.debugconsole(helperimage);
		...
		//this.debugconsole("unread : " + inEvent[0].unread);
	...
	toggleArticleRead: function(inSender, inEvent) {
		...
		//this.debugconsole(Readstate + " " + RecentArticle);
	...
	toggleArticleStarred: function(inSender, inEvent) {
		//this.debugconsole(this.staredon);
	...
	setupCategories: function(inSender, inEvent) {
		//this.debugconsole(inEvent.item);
	...
	selectCategory: function(index) {
		this.setLoadbar(true);
		//this.debugconsole(this.CategoryID[index]);
	...
	selectFeed: function(index) {
		this.setLoadbar(true);
		//this.debugconsole(ArticleID[inEvent.index] + " - " + Articles[inEvent.index]);
	...
	FeedListPageDown: function(first) {
		var step = first ? window.innerHeight/10 : Math.min(Math.abs(this.startY - this.mouseY) / 4, window.innerHeight/10);
		//this.debugconsole("step " + step + " " + window.innerHeight/5 + " " + this.startY + " " + this.mouseY + " " + (this.startY - this.mouseY));
		this.$.articleScroller.setScrollTop(this.$.articleScroller.getScrollTop() + step);
	},
	...
	holdItem: function(inSender, inEvent){
		...
		if ((window.innerWidth < 1024) && (this.ViewMode == 0)) {
			...
			this.$.grabberArticleView.setShowing(false);
			this.$.viewPanels.setDraggable(false);
			this.debugconsole("Disable Panels");
			this.clickItem(" ", inEvent);
			this.resize();
		};
	},
	enablePanels: function(inSender, inEvent){
		this.debugconsole("ENABLE Panels");
		...
		this.$.viewPanels.setIndex(2);
		this.debugconsole("SETPANEL 8");
		this.resize();
		this.$.viewPanels.setDraggable(true);
	},
	clickItem: function(inSender, inEvent){
		this.debugconsole("VIEW MODE: " + this.ViewMode);
		//this.debugconsole(ArticleID[inEvent.index] + " - " + Articles[inEvent.index]);
		...
		if (this.ViewMode == "1") {
			...
			if ((window.innerWidth < 1024) && (this.AutoLockPanels)) {
							this.debugconsole("Meep");
				this.$.viewPanels.setIndex(3);
				this.debugconsole("SETPANEL 9");
				this.$.left2.setShowing(false);
				this.$.middle.setShowing(false);
				if (gblBB10 || gblWebos) {
					this.$.btnUnlockPanels.setShowing(false);
		...
		if (window.innerWidth < 1024) {
			this.$.viewPanels.setIndex(3);
			this.debugconsole("SETPANEL 10");
		} else {
			this.$.viewPanels.setIndex(2);
			this.debugconsole("SETPANEL 11");
		}
	...
	shareArticle: function(inSender, inEvent){
		...
		switch (inEvent.originator.content) {
			case "Twitter":
				...
				//this.debugconsole(this.ArticleData[this.RecentArticleIndex][0].title);
		...
		switch (KeyCode) {
			case 27:// Backgesture abfangen
				...
				switch (WhichPanel) {
					case 3:
						this.$.viewPanels.setIndex(2);
						this.debugconsole("SETPANEL 13");
						break;
					case 2:
						this.$.viewPanels.setIndex(1);
						this.debugconsole("SETPANEL 14");
						break;
				};
	...
	unsubscribeFeedError: function (inSender, inEvent) {
		this.debugconsole("Unsubscribe Error: " + inEvent);
		this.clickRefresh();
	},
	unsubscribeFeedSuccess: function (inSender, inEvent) {
		this.debugconsole("Unsubscribe Success: " + inEvent);
		this.clickRefresh();
		this.$.viewPanels.setIndex(0);
		this.debugconsole("SETPANEL 16");
	},	
	...
	catchtaponlink: function(inSender, inEvent) {
		this.debugconsole(inEvent.target.href);
