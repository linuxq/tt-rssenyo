...
APPID		:= $(VENDOR).$(APP)
PKG			:= ttrss
VERSION		:= 0.0.$(shell git log --pretty=format:'' | wc -l | sed 's/ *//')
DEPLOY		:= deploy/ttrss-enyo
APK			:= ttrss-debug.apk
BAR			:= ttrss-device.bar
ZIP			:= tt-rssenyo.zip
...
${DEPLOY}:
	...
	cp  chrome/* ${DEPLOY}/
	(cd ${DEPLOY} && zip -r ../tt-rssenyo.zip *)
...
install:
	...
	(ls *.bar 2>/dev/null && ${BB10SDK}/dependencies/tools/bin/blackberry-deploy -installApp ${BB10DEVICE} ttrssenyo-${BB10TYPE}.bar -launchApp  -password ${BB10DEVICEPASS})
...
bar: ${DEPLOY}/config.xml
	...
	@cp bb10/config.xml ${DEPLOY}/
	@cp bb10/indexbb10.html ${DEPLOY}/index.html
	@(cd ${DEPLOY} && zip -r ../../${ZIP} *)
